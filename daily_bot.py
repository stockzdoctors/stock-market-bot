...
class SmartFinanceDashboard:
    ...
    def send_to_telegram(self, message):
        ...
        # Your 3 Telegram group chat IDs
        ...
        for cid in chat_ids:
            try:
                ...
                if response.status_code == 200:
                    results.append(f"✅ Successfully sent to group {cid}")
                    success_count += 1
                    print(f"Message sent to {cid}")
                else:
                    results.append(f"❌ Failed to send to {cid}: HTTP {response.status_code}")
                    print(f"Failed to send to {cid}: {response.text}")
                    
            except requests.exceptions.Timeout:
                results.append(f"❌ Timeout error for group {cid}")
                print(f"Timeout error for {cid}")
            except requests.exceptions.ConnectionError:
                results.append(f"❌ Connection error for group {cid}")
                print(f"Connection error for {cid}")
            except Exception as e:
                results.append(f"❌ Error for group {cid}: {str(e)}")
                print(f"Error for {cid}: {e}")
        
        # Return status
        success = success_count == len(chat_ids)
        status_message = "\n".join(results)
        
        return success, status_message
    
    def test_telegram_connection(self):
        """Test connection to all Telegram groups"""
        bot_token = "8259489232:AAH_1aZRNl_0dnJe_ZRA4g3TM9Pj53F148E"
        chat_ids = ["-1001669216683", "-1003702373696", "-1001645367784"]
        
        test_message = "🤖 *Bot Test Message* \n\nIf you receive this, your Smart Finance Dashboard is configured correctly! ✅"
        ...
        for cid in chat_ids:
            try:
                ...
                if response.status_code == 200:
                    results.append(f"✅ Connected to {cid}")
                else:
                    results.append(f"❌ Failed to connect to {cid}: {response.text}")
            except Exception as e:
                results.append(f"❌ Error connecting to {cid}: {str(e)}")
...
def main():
    ...
    with st.expander("🔧 Telegram Settings & Test", expanded=False):
        st.write("**Bot Token:** 8259489232:AAH_1aZRNl_0dnJe_ZRA4g3TM9Pj53F148E")
        st.write("**Group 1 Chat ID:** -1001669216683 (BG 1 Group Sumit)")
        st.write("**Group 2 Chat ID:** -1003702373696 (BG 2 group Monitha)")
        st.write("**Group 3 Chat ID:** -1001645367784")
        
        if st.button("📡 Test Telegram Connection", key="test_telegram"):
            with st.spinner("Testing Telegram connection..."):
                test_result = dashboard.test_telegram_connection()
                st.info(test_result)
    ...
    if st.session_state.get('analysis_done', False):
        ...
        with col1:
            if st.button("🚀 Send to Telegram", type="primary", use_container_width=True):
                with st.spinner("Sending to all Telegram groups..."):
                    success, message = dashboard.send_to_telegram(telegram_message)
                    
                    if success:
                        st.success("✅ Message sent to ALL Telegram groups successfully!")
                        st.balloons()
                    else:
                        st.warning(f"⚠️ Partial Success:\n{message}")
                        st.info("💡 Tip: Make sure bot is added to all groups with send permissions")
